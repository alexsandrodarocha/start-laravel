    ...
    public function reports(Request $request)
    {
        ...
        switch ($report) {
            ...
            case 'user-tag':
                $tagId = $request->query('tagId');
                $userId = $request->query('userId');
                $posts = ReportService::postsByUserAndTag($userId, $tagId);
                return response()->json(['success' => true, 'msg'=>'Relatório retornado com sucesso.', 'data' => $posts]);
            case 'user-activity':
                $userId = $request->query('userId');
                $data = ReportService::userActivity($userId);
                return response()->json(['success' => true, 'msg'=>'Relatório retornado com sucesso.', 'data' => $data]);
            case 'recents':
                $page = $request->query('page');
                $limit = $request->query('limit');
                $data = ReportService::recentPosts($page, $limit);
                return response()->json(['success' => true, 'msg'=>'Relatório retornado com sucesso.', 'data' => $data]);
            default:
